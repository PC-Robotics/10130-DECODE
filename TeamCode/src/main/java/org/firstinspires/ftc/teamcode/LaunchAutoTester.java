...
@Autonomous(name="LaunchAutoTester")
//@Disabled
public class LaunchAutoTester extends RobotBase
{

    final double FEED_TIME = 2.0; //The feeder servos run this long when a shot is requested.
    ...
    final double LAUNCHER_MIN_VELOCITY = 1425;
    ...
    public LaunchAutoTester(LinearOpMode opMode, boolean isFc) {
        super(opMode, isFc);
    }
    ...
    */
    ...
    boolean rotate(double speed, double angle, AngleUnit angleUnit, double holdSeconds){
        ...
        leftRearDrive.setTargetPosition((int) leftTargetPosition);
        rightRearDrive.setTargetPosition((int) rightTargetPosition);
        ...
        if((Math.abs(leftTargetPosition - leftFrontDrive.getCurrentPosition())) > (TOLERANCE_MM * TICKS_PER_MM))//&& Math.abs(leftTargetPosition - leftRearDrive.getCurrentPosition())) > (TOLERANCE_MM * TICKS_PER_MM)){
            driveTimer.reset();
        }
